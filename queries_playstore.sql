...
-- 13. Count all the reviews in the Google Play Store.
    SELECT SUM(reviews) AS "ALL TIME REVIEW COUNT" FROM analytics;

-- 14. Find all the categories that have more than 300 apps in them.
    SELECT category FROM analytics 
        GROUP BY category 
        HAVING COUNT(*) > 300;
    -- SELECT category, COUNT(*)>300 AS over_300 FROM analytics GROUP BY category;
